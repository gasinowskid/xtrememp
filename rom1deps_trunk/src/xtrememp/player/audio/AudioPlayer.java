...
public class AudioPlayer implements Callable<Void> {
    ...
    protected final int READ_BUFFER_SIZE = 8 * 1024;
    ...
    protected void notifyEvent(Playback state, Map properties) {
        for (PlaybackListener listener : listeners) {
            PlaybackEventLauncher launcher = new PlaybackEventLauncher(this, state, getPosition() - oldPosition, properties, listener);
            launcher.start();
        }
        logger.info("{}", state);
    }
