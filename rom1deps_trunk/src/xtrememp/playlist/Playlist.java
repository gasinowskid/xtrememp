...
//    private static Log log = LogFactory.getLog(Playlist.class);
    ...
    public void resort(int[] newSetOfElts) {//Need tests
        PlaylistItem currentItem = getCursorPosition() != -1 ? playlist.get(getCursorPosition()) : null;
//*** Version with Arrays
        PlaylistItem[] oldPlaylist = new PlaylistItem[playlist.size()];
        for (int i = 0; i < oldPlaylist.length; i++) {
            oldPlaylist[i] = playlist.get(i);
        }
        playlist.clear();
        for (int i = 0; i < newSetOfElts.length; i++) {
            playlist.add(oldPlaylist[newSetOfElts[i]]);
        }
//***
//***... Lists
//        List<PlaylistItem> oldPlaylist = new ArrayList<PlaylistItem>(newSetOfElts.length);
//        for (int i = 0; i < newSetOfElts.length; i++) {
//            oldPlaylist.add(playlist.get(i));
//        }
//        playlist.clear();
//        for (int i = 0; i < newSetOfElts.length; i++) {
//            playlist.add(oldPlaylist.get(newSetOfElts[i]));
//        }
//***
        if (currentItem != null) {
            setCursor(playlist.indexOf(currentItem));
        }
    }
