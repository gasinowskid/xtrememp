 ...
 * @author Deano
 */
public class EQVideoInputStream extends TAsynchronousFilteredAudioInputStream{

   private final Logger logger = LoggerFactory.getLogger(EQVideoInputStream.class);

   protected static final int BLOCKSIZE = 17650;
   ...
   private static final int SAMPLING_RATE = 44100;
     ...
     * Audio input buffer.
     */
    private ByteBuffer videoDataIn;
    /**
     * Audio output buffer.
     */
    private ByteBuffer videoDataOut;
   ...
   public EQVideoInputStream(AudioInputStream sourceStream) {
...
//        skipped += bytesToSeconds((int) sk, sourceStream.getFormat());
    ...
    public void execute() {
        ...
        int nBytesRead = 0;
        try {
            nBytesRead = sourceStream.read(videoDataIn.array(), 0,
                    BLOCKSIZE * nChannels);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        byte[] trimBuffer = videoDataIn.array();
        if (nBytesRead < trimBuffer.length) {
            trimBuffer = new byte[nBytesRead];
            System.arraycopy(videoDataIn.array(), 0, trimBuffer, 0, nBytesRead);
        }
...
//        totalTime += bytesToSeconds(nBytesRead, sourceStream.getFormat());
        ...
        if (nBytesRead >= 0) {
            getCircularBuffer().write(videoDataOut.array(), 0, sampleCount * 2);
        }
    ...
    public void init() {
        ...
        videoDataIn = ByteBuffer.allocate(BLOCKSIZE * nChannels);
        videoDataIn.order(ByteOrder.LITTLE_ENDIAN);
        shortDataIn = videoDataIn.asShortBuffer();

        //audioDataOut = ByteBuffer.allocate(BLOCKSIZE);

       
        videoDataOut = ByteBuffer.allocate(BLOCKSIZE * nChannels);


        videoDataOut.order(ByteOrder.LITTLE_ENDIAN);
        shortDataOut = videoDataOut.asShortBuffer();
